/**
 * Spec 49A — R1 gate operator + four client records on tenant r1-gate.
 * Passwords reset every run. Record 1 imports summit-ffm-0625.csv ONLY (never 0617).
 *
 * Usage: npm run test:seed:r1-gate
 * Then:  npx tsx scripts/seed-r1-gate-book.ts   (0625 + SELECTHEALTH suggested)
 */
...
export const R1_GATE_OPERATOR_EMAIL = "r1_gate_operator@codexone.test";
export const R1_GATE_PASSWORD = "r1_gate_2026!";
export const R1_GATE_TENANT_SLUG = "r1-gate";
export const R1_GATE_TENANT_NAME = "R1 Gate";

export const R1_GATE_CLIENTS = [
  { email: "r1_gate_client_1@codexone.test", displayName: "R1 Gate Client 1" },
  { email: "r1_gate_client_2@codexone.test", displayName: "R1 Gate Client 2" },
  { email: "r1_gate_client_3@codexone.test", displayName: "R1 Gate Client 3" },
  { email: "r1_gate_client_4@codexone.test", displayName: "R1 Gate Client 4" },
];
...
async function ensureTenant(admin, operatorUserId) {
  log("Tenant", `lookup slug ${R1_GATE_TENANT_SLUG}`);
  const { data: existing } = await admin
    .from("tenants")
    .select("id, name")
    .eq("domain_slug", R1_GATE_TENANT_SLUG)
    .maybeSingle();
  ...
  const { data: created, error } = await admin
    .from("tenants")
    .insert({
      name: R1_GATE_TENANT_NAME,
      domain_slug: R1_GATE_TENANT_SLUG,
...
async function main() {
  ...
  log("Start", "Spec 49A r1-gate environment");
  ...
  const clientIds = [];
  for (const c of R1_GATE_CLIENTS) {
    const user = await ensureAuthUser(admin, c.email, R1_GATE_PASSWORD, c.displayName);
  ...
  // Isolation: r1-gate operator must not hold summit-test-op grants
  const { data: summitTenant } = await admin
    .from("tenants")
    .select("id")
    .eq("domain_slug", "summit-test-op")
    .maybeSingle();
  if (summitTenant) {
    const { count: leak } = await admin
      .from("client_module_access")
      .select("id", { count: "exact", head: true })
      .eq("distributor_tenant_id", summitTenant.id)
      .eq("granted_by", operator.id);
    if ((leak ?? 0) > 0) {
      throw new Error(
        `Isolation fail: r1-gate operator has ${leak} grants on summit-test-op`
      );
    }
  }

  const { count: gateGrants } = await admin
    .from("client_module_access")
    .select("id", { count: "exact", head: true })
    .eq("distributor_tenant_id", tenantId)
    .eq("status", "active");
  if ((gateGrants ?? 0) !== 4) {
    throw new Error(`Expected 4 active grants on r1-gate, got ${gateGrants}`);
  }

  await verifySignIn(url, anonKey, R1_GATE_OPERATOR_EMAIL, R1_GATE_PASSWORD);
  await verifySignIn(url, anonKey, R1_GATE_CLIENTS[0].email, R1_GATE_PASSWORD);
  await verifySignIn(url, anonKey, R1_GATE_CLIENTS[1].email, R1_GATE_PASSWORD);

  console.log("\n--- Spec 49A credentials ---");
  console.log("Operator:", R1_GATE_OPERATOR_EMAIL, "/", R1_GATE_PASSWORD);
  console.log("Tenant:", R1_GATE_TENANT_SLUG, `(${tenantId})`);
  for (const c of clientIds) {
    console.log(`Client: ${c.email} / ${R1_GATE_PASSWORD}  id=${c.id}  (${c.displayName})`);
  }
  console.log("\nNext: npx tsx scripts/seed-r1-gate-book.ts");
  console.log("(imports 0625 only onto client 1; leaves SELECTHEALTH suggested)");
}
