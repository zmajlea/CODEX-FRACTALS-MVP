-- Spec B16F2 — per-period new_count (label is null) on payee_stats bars.
-- Additive field on by_month / by_week; same filtered matched CTE as count
-- (direction / amount / date / payee) so "new" respects popup filters.
...
begin
  ...
  -- Spec B16F2: new_count lives in the same filtered matched CTE as count
  -- (direction/amount/date/payee already applied above).
  with matched as (
    select abs(t.amount) as mag, t.posted_date, t.label
    from public.treasury_transactions t
    where t.client_user_id = p_client
      and t.is_removed = false
      and t.pending = false
  ...
  by_m as (
    select
      ...
      count(*)::int as count,
      count(*) filter (where label is null)::int as new_count,
