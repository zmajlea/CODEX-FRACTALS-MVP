...
export const BCN_SECTIONS: BcnSectionDef[] = [
  {
    id: "people",
...
export type BcnSectionPayload = Record<string, unknown>;

export type BcnWizardState = {
  sections: Record<string, BcnSectionPayload>;
};

export function emptyWizardState(): BcnWizardState {
  return { sections: Object.fromEntries(BCN_SECTIONS.map((s) => [s.id, {}])) };
}
